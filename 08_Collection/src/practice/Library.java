...
public class Library {
	...
	private List<Book> books;	// 배열은 고정사이즈, 너무 많음
	private Scanner sc;
	...
	private void deleteBook() { //RuntimeException은 unChecked-Exception이므로 throws runtimeException을 생략할 수 있다.
		...
		if(books.isEmpty()) {	// 공백검사 isEmpty
				throw new RuntimeException("등록된 책이 없습니다.");
		}
		System.out.print("삭제할 isbn >>> ");
		String isbn = sc.next();
		if(isbn.isEmpty() == false) {
				/* 1. 객체 기반 삭제(Book 객체 간 동등 비교가 필요하기 때문에 equals 메소드를 오버라이드를 해야 한다.) */
				for(Book book : books) {
				...
				for(int i=0; i<books.size(); i++) {
					...
					if(isbn.equals(book.getIsbn()));		// 향상 for문 쓰면 index 정보 없음
						Book deletedBook = books.remove(i);	// remove(i)는 삭제된 요소(Book)를 반환한다. // Book deletedBook 생략 가능	// remove는 삭제후 알아서 땡겨옴
						System.out.println(deletedBook + " 책이 삭제되었습니다.");						 // 생략하면 deletedBook -> book으로 변경
						return;																			 
				}
			}*/
		}
