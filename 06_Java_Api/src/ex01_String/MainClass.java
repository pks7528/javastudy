...
public class MainClass {
	...
	public static void equals() {
		...
		// 대소문자도 일치해야 함
		if(str1.equals(str2)) {
			System.out.println("srt1과 str2는 같은 문자열이다.");
		} else {
			System.out.println("srt1과 str2는 다른 문자열이다.");
		}
		
		// 대소문자는 무시함
		if(str1.equalsIgnoreCase(str2)) {									// upper case 대문자, lower case 소문자
			System.out.println("srt1과 str2는 같은 문자열이다.");
		} else {
			System.out.println("srt1과 str2는 다른 문자열이다.");
		}
...
//		System.out.println(name.charAt(2));
		...
		for(int i=0; i<name.length(); i++){ 			// <-- 반복문이 name.length() 메소드 호출 계속/// 성능 떨어짐/// 리팩토링 해야함 
	...
	public static void substring() {
		...
		// 2. substring(int begin, int end) : 인덱스 begin부터 end 이전까지 반환(begin <= 추출범위 < end) ** end를 포함하지 않음 **
	...
	public static void contains() {
		...
		String email = "pks7528@naver.com";
