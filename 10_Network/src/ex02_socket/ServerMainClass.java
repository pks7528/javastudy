...
public class ServerMainClass {
	...
	public static void main(String[] args) { //서버는 서버소켓 클리이턴트는 소켓 가지고 있어야함
		...
		try {
			...
			// InetSocketAddress : Socket 사용 시 "호스트", "포트 번호"를 관리하는 클래스
			...
			while(true) {
				...
				// 클라이언트의 접속 요청을 허용				// 클라이언트는 소켓,, 서버는 서버소켓으로 받음
				Socket clientSocket = serverSocket.accept();
				
				// 클라이언트 접속 확인을 위해서 InetSocketAddress를 활용
				InetSocketAddress clienAddress = (InetSocketAddress) clientSocket.getRemoteSocketAddress();
				System.out.println("[서버] 접속된 클라이언트 : " + clienAddress.getHostName());
				
				// 클라이언트에게 welcome 메시지 보내기			//클라이언트 소켓에 데이터를 보내주는 ????
