...
public class XMLMainClass {
	...
	*/
	...
	/*
		목표
			1. 경로
				C:\storage\product.xml		//product.xml 문서 자체를 document라 부름 // 문서에 들어가는 각종 태그는 element // 각자의 구성요소를 객체로 보는 것을 dom처리방식
			2. 내용
				<product>		// 시작태그
					<product>
						<model>세탁기</model>	// 태그 사이에 들어있는 텍스트를 textContent라 한다.
						<maker>삼성</maker>
						<price>100</price>
					</products>
				</products>		// 종료태그
	...
	*/
	...
	public static void main(String[] args) {
		...
		try {
			...
			DocumentBuilderFactory factory = DocumentBuilderFactory.newInstance();	//DBF에서 DB뽑아냄
			...
			for(Map<String, Object> map : productList) {
				...
				price.setTextContent((int)map.get("price") + "");	// setTextContent는 무조건 String으로 캐스팅
			}
