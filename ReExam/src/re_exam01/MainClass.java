...
//			absNumber = - number;
//		}
		int absNumber = (number>=0 ? number : -number); // 3항 연산자
...
//		double area = width * (double) height / 2.0;
//		double area = (double) width * (double) height / 2.0;
	...
	public static void ex04() {
		...
		*/
		...
		int[] lastDay = { 0, 31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31 }; // 0 -> dummy node 아무것도 안하는데 넣은 것
		if(month < 0 || month > 12) {
	...
	public static void ex06() {
		...
		System.out.println(Arrays.toString(unit)); // 배열의 출력을 도와주는 자바 스크립트 Arrays.toString()
